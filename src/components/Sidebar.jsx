...
export default function Sidebar({ onSeleccionar }) {
  const { contactos, contactoActivoId, loadingChats, crearContacto, editarContacto, eliminarContacto } = useChat()
  ...
  function handleAbrirCrearContacto() {
    setModalContact(null)
    setFormError('')
    setShowModal(true)
  }

  function handleAbrirEditarContacto(contacto) {
    setModalContact(contacto)
    setFormError('')
    setShowModal(true)
  }

  async function handleGuardarContacto(payload) {
    setSavingContact(true)
    setFormError('')

    try {
      if (modalContact) {
        await editarContacto(modalContact.id, payload)
      } else {
        const contacto = await crearContacto(payload)
        onSeleccionar(contacto.id)
      }
  ...
  async function handleEliminarContacto(contacto) {
    const confirmar = window.confirm(`Eliminar a ${contacto.name}? Tambien se borran sus mensajes.`)
    if (!confirmar) return
  ...
  const iconBtn = {
    ...
    color: 'var(--icon-color)', transition: 'background 0.15s',
  }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', flexShrink: 0 }}>
      ...
      {/* Barra izquierda */}
      <div style={{
        width: '60px', background: 'var(--bg-sidebar)',
...
<div style={{
  ...
  flexShrink: 0,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ color: 'var(--accent)', fontWeight: '600', fontSize: '18px' }}>WhatsApp</span>
          </div>
          <div style={{ display: 'flex', gap: '4px' }}>
            <button onClick={handleAbrirCrearContacto} title="Nuevo contacto" style={{ ...iconBtn, width: '36px', height: '36px' }}
              onMouseEnter={e => e.currentTarget.style.background='var(--bg-hover)'}
              onMouseLeave={e => e.currentTarget.style.background='transparent'}>
...
{/* Buscador */}
<div style={{ padding: '8px 12px', flexShrink: 0, background: 'var(--bg-sidebar)' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', padding: '4px 12px 8px', flexShrink: 0 }}>
          {['Todos', 'No leídos', 'Favoritos'].map((f) => (
            <button key={f} onClick={() => setFiltro(f)} style={{
              ...
              background: filtro === f ? 'var(--bg-active)' : 'var(--bg-header)',
              color: filtro === f ? 'var(--accent)' : 'var(--text-secondary)',
          ...
          {loadingChats ? (
            ...
            filtrados.map(contacto => (
              <ContactItem
                ...
                onEdit={() => handleAbrirEditarContacto(contacto)}
                onDelete={() => handleEliminarContacto(contacto)}
              />
            ))
          )}
        </div>
